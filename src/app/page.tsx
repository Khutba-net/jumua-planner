...
const t = {
  ...
  ar: {
    ...
    salam: "السلام عليكم ورحمة الله وبركاته",
    heroTitle: <>عامٌ من الخطب <span className="text-primary">يشدّ بعضه بعضاً.</span></>,
    heroAccent: "",
    heroSub: <><span className="font-semibold text-ink">«مخطِّط الجمعة»</span>{" نظامٌ لتخطيط خطب الجمعة يأخذ بيد الخطيب إلى الترتيب: تضع "}<span className="font-semibold text-ink">عناوين عامك الرئيسية</span>{" أولاً، ثم تقسم كل عنوان إلى "}<span className="font-semibold text-ink">باقاتٍ فرعية</span>{"، ثم تسمّي "}<span className="font-semibold text-ink">الخطب</span>{" تحتها — فلا تنتقل من موضوع إلى موضوع بغير رابط."}<br /><br />{"فتخرج "}<span className="font-semibold text-ink">خطةٌ يتابعها جمهورك</span>{": خطٌّ واحد متصل من أول العام إلى آخره، مكتوبةٌ خطبه ومُتدرَّبٌ على إلقائها قبل أن تهلّ الجمعة."}</>,
    ...
    obj1Text: <>{"خطِّط عامك كاملاً في جلسة واحدة. يقسم النظام عامك إلى "}<span className="font-semibold text-primary">٤</span>{" فصول، "}<span className="font-semibold text-primary">٣</span>{" أشهر أو "}<span className="font-semibold text-primary">١٣</span>{" أسبوعاً لكل فصل. تسمّي لكل فصل "}<span className="font-semibold text-ink">عنواناً رئيسياً</span>{"، وتختار تحته "}<span className="font-semibold text-primary">٤</span>{" باقات فرعية، ثم تسمّي "}<span className="font-semibold text-primary">٤</span>{" خطب تحت كل باقة. والعيد وسائر المناسبات الهجرية قلّ أن تقع يوم جمعة؛ فتأخذ خطبها موضعها في الخطة في أي يوم وقعت فيه، أسبوعياً كان أو في نهاية الأسبوع."}</>,
    obj1Result: <>{"= "}<span className="font-semibold text-primary">٦٤</span>{" خطبة في السنة، نحو "}<span className="font-semibold text-primary">١٣</span>{" جمعة لكل عنوان رئيسي، بالإضافة إلى خطب المناسبات في أي يوم."}</>,
    ...
    obj2Text: <><span className="font-semibold text-ink">{"اكتُبها، ثم ألقِها بغير ورقة."}</span>{" تحملك نافذة الخطبة على توثيق محتوى كل خطبة وضبطها في حدود "}<span className="font-semibold text-primary">١٥</span>{" إلى "}<span className="font-semibold text-primary">٢٠</span>{" دقيقة؛ وهو قيدٌ واقعي في البلدان التي لا تعطّل الدوام يوم الجمعة وتحدّد للعاملين فسحةً معلومة للخروج والعودة."}<br /><br />{"ثم تتدرّب على إلقائها بغير ورقة: "}<span className="font-semibold text-ink">{"معدٌّ في المعنى، متحرّرٌ من النص."}</span>{" بهذا يتمكّن الخطيب من خطبته، وتبلغ خطبته من سامعها."}</>,
    ...
    smartTitle: <>{"نظامٌ ذكي: لا ينظّم فحسب؛ بل "}<span className="text-primary">يعالج</span>{"، و"}<span className="text-primary">يراقب</span>{"، و"}<span className="text-primary">يرفع التقارير</span></>,
    smartText: <>{"فتبقى خطتك على مسارها طوال العام. وللمشترك أن ينتقل بين المستويات — "}<span className="font-semibold text-ink">الفردي</span>{"، و"}<span className="font-semibold text-ink">المنظّمي</span>{"، و"}<span className="font-semibold text-ink">الأوقاف</span>{" — كلما اتّسعت حاجته."}</>,
    ...
    ctaSub: <>{"خطّط لـ "}<span className="font-semibold text-primary">٦٤</span>{" خطبة عبر "}<span className="font-semibold text-primary">٤</span>{" فصول. اكتب وأعدّ وألقِ كل خطبة بهدف."}</>,
    ...
    why64Title: <>تملك <span className="text-primary">أربعاً وستين</span> فرصة.</>,
    why64Text1: <>{"اثنان وخمسون أسبوعاً، واثنتان وخمسون خطبة جمعة، وتُزاد عليها "}<span className="font-semibold text-ink">اثنتا عشرة خطبة</span>{" للمناسبات الهجرية: "}<span className="font-semibold text-ink">عيد الفطر</span>{"، و"}<span className="font-semibold text-ink">عيد الأضحى</span>{"، و"}<span className="font-semibold text-ink">الهجرة النبوية</span>{"، و"}<span className="font-semibold text-ink">الإسراء والمعراج</span>{"، و"}<span className="font-semibold text-ink">معركة بدر</span>{"... الخ تأتي هذه في تواريخها، ولا تندرج بالضرورة تحت عنوانٍ من عناوينك أو باقةٍ من باقاتك. "}<span className="font-semibold text-primary">٥٢ + ١٢ = ٦٤</span>{"."}</>,
